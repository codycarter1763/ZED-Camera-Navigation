...
def handle_set_home():
    ...
    Captures current ZED pose and saves it as origin_T0.json.
    The drone tmux session must be running (window 0 = ZED active).
    Runs set_origin.py via tmux window 2 which listens for
    /zed/zed_node/pose and writes origin_T0.json on SPACE keypress.
    Here we trigger it programmatically.
    """
    print("  SET HOME — triggering set_origin via tmux window 2")

    if not _tmux_running():
        print("  ERROR: tmux session not running — launch first!")
        return

    # Window 2 is set_origin.py — send SPACE to trigger save
    # (set_origin.py saves pose on spacebar press)
    subprocess.run(
        f'tmux send-keys -t {TMUX_SESSION}:2 " " ""',
        shell=True)
    print("  SPACE sent to set_origin.py (window 2)")

    # Wait briefly then check if file was written
    def _confirm():
        time.sleep(3)
        if os.path.exists(ORIGIN_FILE):
            print(f"  origin_T0.json confirmed ✓")
            send_status(STATUS_HOME_SET)
        else:
            print("  WARNING: origin_T0.json not found after 3s")
            # Send anyway — set_origin may write asynchronously
            send_status(STATUS_HOME_SET)
