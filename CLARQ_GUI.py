...
#
# Gimbal control (nRF path):
#   GUI → Arduino → NRF24L01 → Jetson → /dev/ttyACM2 → SimpleBGC
#
...
# ── Jetson response map (Arduino RX → status label) ───────────
JETSON_STATUS_MAP = {
    "CLARQ_PONG":              ("ONLINE ✓",          "#1ec882", "--"),
    "CLARQ_HOME_SET":          ("HOME SET ✓",        "#1ec882", "origin_T0.json saved"),
    "CLARQ_SCAN_STARTED":      ("SCANNING ✓",        "#1ec882", "Position tracking running"),
    ...
    "CLARQ_LAND_ACK":          ("LANDING...",        "#dc6438", "AprilTag + return_land"),
}
...
class DroneGUI:
    def __init__(self, root):
        ...
        # Gimbal state (angle tracking only — control goes via RF)
    ...
    def gimbal_set_pitch(self, angle):
        """
        Send gimbal angle command via RF → Jetson → SimpleBGC serial.
        The Jetson handles the actual serial communication to the gimbal.
        """
        self.log(f"Gimbal → {angle}° (sending via RF)", "cmd")
    ...
    def set_gimbal_angle(self):
        """Set gimbal angle from the entry field."""
        try:
            angle = float(self.gimbal_angle_entry.get())
            self.gimbal_set_pitch(angle)
        except ValueError:
            self.log("Invalid angle — enter a number", "error")

    def quick_set_angle(self, angle):
        """Quick preset angle button handler."""
    ...
    def build_buttons(self, parent):
        ...
        # ── Gimbal control (via RF → Jetson → SimpleBGC) ──────
        section("GIMBAL CONTROL (via RF → Jetson)")

        tk.Label(
            parent, text="Jetson controls gimbal over serial",
            font=("Consolas", 9, "italic"),
            bg=PANEL, fg=DIM
        ).pack(anchor=tk.W, padx=15, pady=(0, 6))

        # Angle entry + SET button
        ...
        self.gimbal_angle_entry.bind(
            '<Return>', lambda e: self.set_gimbal_angle())
        ...
        # Quick angle presets — positive (up/forward)
        quick_frame1 = tk.Frame(parent, bg=PANEL)
        quick_frame1.pack(fill=tk.X, padx=15, pady=(0, 2))
        for angle in [0, 30, 45, 60, 90]:
            tk.Button(
                quick_frame1, text=f"{angle}°",
                font=("Consolas", 10),
                bg="#2a2a3a", fg=TEXT,
                relief=tk.FLAT, cursor="hand2",
        ...
        # Quick angle presets — negative (down)
        quick_frame2 = tk.Frame(parent, bg=PANEL)
        quick_frame2.pack(fill=tk.X, padx=15, pady=(0, 4))
        for angle in [-30, -45, -60, -90]:
            tk.Button(
                quick_frame2, text=f"{angle}°",
                font=("Consolas", 10),
                bg="#2a2a3a", fg=TEXT,
                relief=tk.FLAT, cursor="hand2",
                command=lambda a=angle: self.quick_set_angle(a)
        ...
        # ── AprilTag ──────────────────────────────────────────
        section("APRILTAG")
        btn("START APRILTAG", GREEN, self.start_tag)
        btn("STOP APRILTAG",  RED,   self.stop_tag)
        ...
        def _connect():
            ...
            try:
                ...
                if ready:
                    ...
                    self.log(f"RF bridge connected on {port} ✓", "info")
                    self.root.after(
            ...
            try:
                if self.ser.in_waiting:
                    ...
                    if line.startswith("TX_OK:"):
                        ...
                        self.log(f"JETSON ACK: {label}", "info")
    ...
    def send_jetson_cmd(self, cmd_id, text):
        ...
        Arduino transmits 2-byte packet over NRF24L01 to Jetson.
        """
        ...
        try:
            ...
            self.log(f"→ Arduino: {packet.strip()}", "dim")
        except Exception as e:
    ...
    def connect_mav(self):
        def _connect():
            try:
                ...
                self.log("MAVLink connected — telemetry active ✓", "info")
                self.root.after(
                ...
                self.log(f"MAVLink connection failed: {e}", "error")
        ...
        while self.telem_on and self.mav_connected:
            try:
                ...
                if t == 'HEARTBEAT':
                    ...
                    self.update_telem('Armed', 'YES' if armed else 'NO')
    ...
    def set_mode(self, mode):
        ...
        def _watch_ack():
            try:
                ...
                pass
        threading.Thread(target=_watch_ack, daemon=True).start()
    ...
    def start_3d_fusion(self):
        ...
        self.log("  Jetson starting ZED Fusion + position tracking", "nav")
    ...
    def stop_3d_fusion(self):
        ...
        self.log("  Saving 3D reconstruction + position data", "nav")
        self.log("─" * 44, "nav")
    ...
    def start_photo_capture(self):
        ...
        self.log("─" * 44, "nav")
        self.send_jetson_cmd(CMD_ID_START_PHOTO, CMD_START_PHOTO)
    ...
    def start_video_capture(self):
        ...
        self.log("─" * 44, "nav")
        self.send_jetson_cmd(CMD_ID_START_VIDEO, CMD_START_VIDEO)

    def stop_capture(self):
        self.log("─" * 44, "nav")
        self.log("  STOPPING CAPTURE — saving files", "nav")
        self.log("─" * 44, "nav")
    ...
    def start_tag(self):
        self.send_jetson_cmd(CMD_ID_START_TAG, CMD_START_APRILTAG)
